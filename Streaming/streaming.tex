...
%	
	...
	In prior sections, we showed the validity of the streaming equation and how different components. However we would like to have a phenomenological model to describe the relative line-of-sight velocity and use that in the streaming equation. GH is the mixture modelling consisting of normal distribution and Generalised Inverse Gaussian distribution. It is similar to modelling pairwise velocity as mixture of the normal and log-normal density (see tinker). GIG and log-normal are very similar. Try to connect them and see. Density weighted velocity, so would make sense to connect them. Could explain why we use it and why they could be the true distribution.
